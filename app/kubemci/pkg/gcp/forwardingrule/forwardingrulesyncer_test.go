...
func TestListLoadBalancerStatuses(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]status.LoadBalancerStatus{
				lbName: status.LoadBalancerStatus{
					"",
					lbName,
					clusters,
					ipAddr,
		...
		{
			// Load balancer should be listed only once, when both http and https
			// forwarding rules exist for the same load balancer.
			[]ensureFR{
				{
					lbName,
					ipAddr,
		...
		},
		{
			...
			map[string]status.LoadBalancerStatus{
				lbName + "1": status.LoadBalancerStatus{
					"",
					lbName + "1",
					clusters,
					ipAddr,
				},
				lbName + "2": status.LoadBalancerStatus{
					"",
					lbName + "2",
					clusters,
					ipAddr,
	...
	for i, c := range testCases {
		glog.Infof("Test case: %d", i)
		frp := ingresslb.NewFakeLoadBalancers("" /*name*/, nil /*namer*/)
		frs := NewForwardingRuleSyncer(namer, frp)
		// Create all forwarding rules.
		for _, fr := range c.forwardingRules {
			if fr.https {
				if err := frs.EnsureHttpsForwardingRule(fr.lbName, fr.ipAddr, fr.tpLink, fr.clusters, false); err != nil {
					t.Errorf("expected no error in ensuring https forwarding rule, actual: %v", err)
				}
			} else {
				if err := frs.EnsureHttpForwardingRule(fr.lbName, fr.ipAddr, fr.tpLink, fr.clusters, false); err != nil {

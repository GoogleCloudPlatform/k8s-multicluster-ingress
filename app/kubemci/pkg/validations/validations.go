...
func ServerVersionsNewEnough(clients map[string]kubeclient.Interface) (bool, error) {
	for key := range clients {
		...
		if discoveryClient == nil {
			return false, fmt.Errorf("no discovery client in %s client: %+v", key, clients[key])
		}
		ver, err := discoveryClient.ServerVersion()
		if err != nil {
			return false, fmt.Errorf("could not get discovery client to lookup server version: %s", err)
		}
		...
		newEnough, err := serverVersionNewEnough(major, minor, patch)
		if err != nil {
			return false, fmt.Errorf("error checking server version: %s", err)
		}
		if !newEnough {
			fmt.Printf("Cluster %s (ver %d.%d.%d) is not running a supported kubernetes version. Need >= 1.8.1 and not 1.10.0.\n",

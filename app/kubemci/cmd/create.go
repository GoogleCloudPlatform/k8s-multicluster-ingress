...
func runCreate(options *CreateOptions, args []string) error {
	...
	clusters, clients, err := createIngress(options.KubeconfigFilename, &ing)
	if err != nil {
		return err
	}
...
func createIngress(kubeconfig string, ing *v1beta1.Ingress) ([]string, map[string]kubeclient.Interface, error) {
...
func unmarshallAndApplyDefaults(filename string, ing *v1beta1.Ingress) error {
	...
	if class == "" {
		...
		return fmt.Errorf("error definining ingress class to %v, must be %v", class, annotations.GceMultiIngressClass)
	}
	return nil
}

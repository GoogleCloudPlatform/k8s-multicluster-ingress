...
# Make sure goveralls is installed.
	...
	@CI_NAME="prow" CI_BUILD_NUMBER=${BUILD_ID} CI_BUILD_URL="no-build-url-plumbed" CI_BRANCH=${PULL_BASE_REF} CI_PULL_REQUEST=${PULL_NUMBER} goveralls -repotoken $(shell cat /etc/coveralls-token/coveralls.txt)
